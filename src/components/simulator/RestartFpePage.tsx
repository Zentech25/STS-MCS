...
import { Power, Plug, MonitorSmartphone } from "lucide-react";
...
const ACCENT = "120 60% 40%";

export function RestartFpePage() {
  const [units, setUnits] = useState<FPEUnit[]>(FPE_UNITS);

  const handleShutdown = (id: string) => {
    setUnits((prev) =>
      prev.map((u) => (u.id === id ? { ...u, status: "offline" as const } : u))
    );
  ...
  const handleReconnect = (id: string) => {
    setUnits((prev) =>
      prev.map((u) => (u.id === id ? { ...u, status: "online" as const } : u))
    );
              ...
              <AlertDialog>
                ...
                <AlertDialogContent className="rounded-2xl">
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      This will immediately shut down {unit.label}. Any active session on this unit will be terminated. You can reconnect it later.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
