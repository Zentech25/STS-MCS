    ...
    <div className="grid grid-cols-12 gap-4 p-4 h-full auto-rows-fr">
      <DashboardPanel title="Simulation Control" className="col-span-4" glow>
        <div className="flex flex-col gap-3">
          ...
          <div className="flex gap-2">
            <button onClick={() => setSimState("running")} disabled={simState === "running"} className={`${btnBase} bg-success/10 border-success/30 text-success hover:bg-success/20 hover:shadow-[0_0_12px_hsl(152_69%_41%/0.2)]`}>
      ...
      <DashboardPanel title="Performance Metrics" className="col-span-4">
        <div className="space-y-4">
          {[
            { label: "Accuracy", value: 87, icon: <Target className="w-3.5 h-3.5" />, color: "hsl(199 89% 48%)" },
            { label: "Response Time", value: 72, icon: <Activity className="w-3.5 h-3.5" />, color: "hsl(270 60% 58%)" },
            { label: "Tactical Score", value: 91, icon: <BarChart3 className="w-3.5 h-3.5" />, color: "hsl(330 65% 55%)" },
          ].map((m) => (
      ...
      <DashboardPanel title="Trainee Status" className="col-span-4">
        <div className="space-y-1.5">
          {trainees.map((t) => (
            <div key={t.name} className="flex items-center justify-between p-2.5 rounded-lg bg-secondary/30 border border-border/20 hover:bg-secondary/50 hover:border-border/40 transition-all duration-200">
